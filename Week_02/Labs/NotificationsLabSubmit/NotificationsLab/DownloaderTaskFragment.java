...
public class DownloaderTaskFragment extends Fragment {
	...
	public class DownloaderTask extends AsyncTask<ArrayList<Integer>, Void, String[]> {
		...
		// Uncomment this helper method. If necessary, notifies the user that the tweet downloads are complete. Sends an ordered broadcast back to the BroadcastReceiver in MainActivity to determine whether the notification is necessary.
		private void notify(final boolean success) {
			...
			// Sends an ordered broadcast to determine whether MainActivity is active and in the foreground. Creates a new BroadcastReceiver to receive a result indicating the state of MainActivity
			...
			mContext.sendOrderedBroadcast(new Intent(MainActivity.DATA_REFRESHED_ACTION), null, new BroadcastReceiver() {
				...
				public void onReceive(Context context, Intent intent) {
					...
					if (getResultCode() != MainActivity.IS_ALIVE) {
						// TODO: If not, create a PendingIntent using the restartMainActivityIntent and set its flags to FLAG_UPDATE_CURRENT
						...
						// TODO: Set the notification View's text to reflect whether the download completed successfully
						if (success) {
							mContentView.setTextViewText(R.id.text, successMsg);
						} else {
							mContentView.setTextViewText(R.id.text, failMsg);
						}
						// TODO: Use the Notification.Builder class to create the Notification. You will have to set several pieces of information. You can use android.R.drawable.stat_sys_warning for the small icon. You should also setAutoCancel(true).
						Notification.Builder notificationBuilder = new Notification.Builder(mContext);
						notificationBuilder.setContentIntent(pendingIntent).setContent(mContentView).setSmallIcon(android.R.drawable.stat_sys_warning).setAutoCancel(true);
		...
		// Uncomment this helper method. Saves the tweets to a file.
